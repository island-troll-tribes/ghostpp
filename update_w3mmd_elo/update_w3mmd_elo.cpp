				...
				if( !ignore )
				{
					...
					else
					{
						...
						for( int i = 0; i < num_players; i++ )
						{
							...
							float ratingchange = player_ratings[i] - old_player_ratings[i];
							string UpdateScoreChange = "UPDATE w3mmdplayers SET elochange=" + UTIL_ToString( ratingchange, 2 ) + " WHERE id=" + UTIL_ToString( elochangeids[i] );
							if( mysql_real_query( Connection, UpdateScoreChange.c_str( ), UpdateScoreChange.size( ) ) != 0 )
							{
								cout << "error: " << mysql_error( Connection ) << endl;
								return 1;
							}
